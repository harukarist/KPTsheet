...
const dateCol = 2; //日付列
let lastRow = formSheet.getLastRow(); //最終行
let range = formSheet.getRange(2,startCol,lastRow,numCols); //対象範囲
...
function formatCells(){
  ...
  formSheet.getRange(2,dateCol,lastRow, 1).setNumberFormat('MM/dd (aaa)'); //日付の表示形式を変更
}